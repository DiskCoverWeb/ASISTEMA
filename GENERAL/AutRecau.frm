   ...
   Begin MSComctlLib.Toolbar Toolbar1 
      ...
      BeginProperty Buttons {66833FE8-8583-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty Button8 {66833FEA-8583-11D1-B16A-00C0F0283628} 
            Key             =   "BajarExcel"
            Object.ToolTipText     =   "Descargar a Excel"
            ImageIndex      =   10
         EndProperty
      EndProperty
      BorderStyle     =   1
      Begin MSDataListLib.DataCombo DCGrupoF 
         Bindings        =   "AutRecau.frx":4F0A
         DataSource      =   "AdoGrupo"
         Height          =   360
         Left            =   12075
         TabIndex        =   5
      ...
      Begin VB.Frame Frame3 
         Caption         =   "ENTIDAD FINANCIERA(BANCARIA)"
         BeginProperty Font 
            Name            =   "MS Sans Serif"
            Size            =   8.25
            Charset         =   0
...
'''              End If
         Case "PRODUBANCO"
              CodigoP = CamposFile(8).Valor
              CodigoCli = CamposFile(8).Valor
              NombreCliente = CamposFile(9).Valor
              SubTotal = Val(Replace(CamposFile(11).Valor, "$", ""))
              Total = SubTotal
              FechaTexto = CamposFile(3).Valor
              Producto = CamposFile(16).Valor
              CodigoB = CamposFile(16).Valor
              
              NoAnio = Val(SinEspaciosDer(CodigoB))
              CodigoB = ULCase(TrimStrg(MidStrg(CodigoB, 1, Len(CodigoB) - 4)))
              Mes = SinEspaciosDer(CodigoB)
              NoMeses = LetrasMeses(Mes)
              CodigoInv = SinEspaciosIzq(CodigoB)
              
             'Este Valor es Provisional hasta que se suba la carga por DiskCover
              If NoMeses = 1 And CodigoInv = "Matricula" Then NoMeses = 4
             '------------------------------------------------------------------
              Codigo3 = CamposFile(19).Valor
              Codigo4 = CamposFile(29).Valor
              Codigo2 = CamposFile(25).Valor
...
'''         Case "TARJETAS"
'''              If UBound(CamposFile) >= 23 Then
'''                 Cantidad = 1
'''                 CodigoCli = CamposFile(8).Valor
'''                 NombreCliente = Ninguno
'''                 NumeroTarjeta = CamposFile(10).Valor
'''                 SubTotal = Val(CamposFile(17).Valor)
'''                 CostoTarjeta = Val(CamposFile(20).Valor)
'''                 Total = Val(CamposFile(23).Valor)
...
'''
'''                'MsgBox "Desktop Test: " & CodigoB & vbCrLf & CodigoP & vbCrLf & NombreCliente & vbCrLf & SubTotal
'''
'''                 CodigoB = LCase(TrimStrg(MidStrg(TrimStrg(SinEspaciosDer(CodigoB)), 6, 3)))
'''                  'LetrasMeses(CodigoB)
'''
'''                 Codigo3 = "Tarjeta No."
'''                 Codigo4 = NumeroTarjeta
'''                 Codigo2 = CamposFile(7).Valor
'''              End If
'''             'MsgBox "Desktop Test: " & CodigoCli & vbCrLf & CodigoP & vbCrLf & NombreCliente & vbCrLf & FechaTexto & vbCrLf & Total & vbCrLf & Saldo & vbCrLf & NoAnio & vbCrLf & NoMeses
         Case ".."
               CodigoCli = CamposFile(0).Valor
               FechaTexto = CStr(CamposFile(1).Valor)
               NoMeses = Val(CamposFile(2).Valor)
...
'''                  End If
      ...
      'MsgBox NombreCliente & vbCrLf & Len(CodigoCli) & vbCrLf & CodigoCli
       ...
       If CodigoInv <> Ninguno Then
          ...
          sSQL = "SELECT Producto, IVA, Cta_Ventas, Cta_Ventas_0, Cta_Ventas_Ant " _
               & "FROM Catalogo_Productos " _
               & "WHERE Item = '" & NumEmpresa & "' " _
               & "AND Periodo = '" & Periodo_Contable & "' " _
          ...
          If AdoAux.Recordset.RecordCount > 0 Then
             Producto = AdoAux.Recordset.fields("Producto")
             If Len(AdoAux.Recordset.fields("Cta_Ventas_Ant")) > 1 Then
                Cta_Ventas = AdoAux.Recordset.fields("Cta_Ventas_Ant")
             Else
                If AdoAux.Recordset.fields("IVA") Then Cta_Ventas = AdoAux.Recordset.fields("Cta_Ventas") Else Cta_Ventas = AdoAux.Recordset.fields("Cta_Ventas_0")
             End If
             'MsgBox "Desktop Test: "
          End If
       End If
    ...
    '''   MsgBox Cadena
      'Verificamos la primera deuda antgua que tenga el Cliente de ese mes
       If IsDate(FechaTexto) Then
          ...
          'MsgBox "Desktop Test: " & FA.Cod_CxC
             ...
             Do While Not AdoAux.Recordset.EOF
               ...
               'MsgBox "Codigo: " & CodigoCli & ", De: " & NombreCliente & " Por = " & Format(Total, "#,##0.00") & vbCrLf
                If Total <= Sumatoria Then
                   ...
                   SetAdoFields "NoMes", NoMeses
                   SetAdoFields "Serie_No", NoAnio
                   ...
                   SetAdoFields "Codigo_B", FA.Cod_CxC
                   SetAdoFields "Procedencia", TextoBanco
                   'If NumeroTarjeta <> "" Then SetAdoFields "Serie_No", NumeroTarjeta
   ...
   If .RecordCount > 0 Then
       Do While Not .EOF
          ...
          sSQL = "SELECT Codigo,Num_Mes,Periodo,SUM(Valor) AS TValor " _
               ...
               & "AND Num_Mes = " & .fields("NoMes") & " " _
               & "AND Periodo <= '" & .fields("Serie_No") & "' " _
...
Dim CostoTarjeta As Currency
   ...
   On Error GoTo ManejoErrores ' Salta a la etiqueta si hay un error
   Progreso_Barra.Mensaje_Box = "SUBIENDO ABONOS DEL BANCO " & TextoBanco
   Progreso_Iniciar
   DGFactura.Visible = False
   TxtFile = ""
   ...
   If FA.ResultadoFA = "OK" Then
     ...
     'FA.Factura = Numero_Factura(FA)
      ...
      Cta_Bancaria = SinEspaciosIzq(DCBanco)
      CaptionTemp = FRecaudacionBancosPreFa.Caption
      TextoImprimio = ""
      Separador = ","
      ...
      CDialogDir.CancelError = True ' ¡Importante!
      CDialogDir.Filename = RutaSysBases & "\BANCO\ABONOS\*.*"
      CDialogDir.InitDir = RutaSysBases & "\BANCO\ABONOS\"
      CDialogDir.Flags = cdlOFNFileMustExist + cdlOFNNoChangeDir + cdlOFNHideReadOnly
      CDialogDir.Filter = "Archivos PDF|*.txt"
      ...
      CDialogDir.ShowOpen
'      CDialogDir.Action = 1
      ' Verificar si hubo un error (si se pulsó Cancelar)
      NombreArchivo = CDialogDir.Filename
     '--------------------------------------------------------------------
      If NombreArchivo <> "" Then
...
ManejoErrores:
    ' Este bloque se ejecuta si se produjo un error (ej. Cancelar)
    If Err.Number = cdlCancel Then ' O simplemente If Err.Number <> 0 Then
  ...
  Select Case Tipo_Carga
    ...
    Case Else
         sSQL = "SELECT C.Codigo As CodigoC,F.Codigo,C.Grupo,C.Cliente,C.CI_RUC,C.Direccion,C.Casilla,C.Actividad,C.Plan_Afiliado,F.Periodo,F.Num_Mes,F.Fecha," _
              ...
              & "F.Codigo_Inv, CC.Producto, C.Saldo_Pendiente,SUM(F.Valor-(F.Descuento+F.Descuento2)) As Valor_Cobro "
         ...
         sSQL = sSQL _
              ...
              & "AND CC.Periodo = '" & Periodo_Contable & "' "
  ...
  Select Case Tipo_Carga
    ...
    Case Else
         sSQL = sSQL _
              ...
              & "AND F.Codigo_Inv = CC.Codigo_Inv " _
              & "AND F.Item = CC.Item " _
              & "GROUP BY C.Codigo,F.Codigo,C.Grupo,C.Cliente,C.CI_RUC,C.Direccion,C.Casilla,C.Actividad,C.Plan_Afiliado,F.Periodo,F.Codigo_Inv, CC.Producto, F.Num_Mes,F.Fecha," _
...
Private Sub Generar_Facturas_PreFA()
...
Dim Cod_Ok As Boolean
   ...
   Mensajes = "Esta Seguro que desea grabar: " & vbCrLf _
            & "desde La Factura Preprocesada " & TextFacturaNo
...
'''              TBenef = Leer_Datos_Cliente_SP(CodigoCli)
...
''''''              Select_Adodc AdoAux, sSQL
'''
'''              sSQL = "SELECT CF.*,AF.Codigo_Cliente,AF.CODIGO_L,AF.TICKET,AF.PRODUCTO,AF.HABIT, AF.Serie_No, AF.A_No  " _
...
'''                 CodigoT = AdoProducto.Recordset.fields("TICKET")
...
'''                       End If
'''
'''                       If Len(DatInv.Cta_Venta_Anticipada) > 1 Then
'''                          Cta_Ventas = DatInv.Cta_Venta_Anticipada
'''                       Else
'''                          If FA.TC = "NV" Then
'''                             Cta_Ventas = DatInv.Cta_Ventas_0
'''                          Else
'''                             If Len(DatInv.Cta_Ventas_Ant) > 1 Then
'''                                Cta_Ventas = DatInv.Cta_Ventas_Ant
'''                             Else
'''                                If DatInv.IVA Then Cta_Ventas = DatInv.Cta_Ventas Else Cta_Ventas = DatInv.Cta_Ventas_0
'''                             End If
'''                          End If
'''                       End If
...
'''                       SetAdoUpdate
'''                      'MsgBox "Desktop Test: " & Factura_No & vbCrLf & CodigoCli & vbCrLf & AdoProducto.Recordset.fields("Num_Mes") & vbCrLf & AdoProducto.Recordset.fields("Periodo") & vbCrLf & AdoProducto.Recordset.fields("Mes")
...
'''                        If CodigoT <> Ninguno Then TipoDeAbono = TipoDeAbono & "No. " & CodigoT
'''                   Case Else
'''                        TipoDeAbono = "DEPOSITO EN EL BANCO "
'''                 End Select
...
'''       End If
'''      End With
      Grabar_Facturas_x_PreFA_SP FA
      FechaTexto = DateSerial(Left(FA.Fecha, 4), Mid(FA.Fecha, 5, 2), Right(FA.Fecha, 2))
   End If
...
'''   Ejecutar_SQL_SP sSQL
   ...
   RatonNormal
   Progreso_Final
   LabelAbonos.Caption = Format$(FA.SubTotal, "#,##0.00")
   MsgBox "GENERACION DE FACTURAS DEL: " & FechaTexto & vbCrLf _
        & "SE GRABARON: " & FA.CantFact & " CLIENTES." & vbCrLf _
        & "DESDE LA " & FA.TC & ": " & FA.Desde & " HASTA LA " & FA.Hasta & vbCrLf _
...
Private Sub Form_Load()
 'CentrarForm FRecaudacionBancosPreFa
  ...
  DGFactura.Height = DGFactura.Top - 2500
  ...
  Command2.Top = DGFactura.Top + DGFactura.Height + 10
  Label4.Top = DGFactura.Top + DGFactura.Height + 10
  Label4.Left = AdoFactura.Left + AdoFactura.width
  LabelAbonos.Top = DGFactura.Top + DGFactura.Height + 10
  LabelAbonos.Left = AdoFactura.Left + AdoFactura.width + Label4.width
End Sub
...
With AdoFactura.Recordset
 If .RecordCount > 0 Then
    .MoveFirst
     ...
     Codigo3 = TrimStrg(MidStrg(Empresa, 1, 30))
     Total = 0
     TotalIngreso = 0
     Total_Factura = 0
     IE = 1
      ...
      .MoveFirst
       ...
       Do While Not .EOF
          ...
          If .fields("Tipo_Cta") = "TARJETA" And Len(TRep.Cta_Numero) >= 14 Then
             ...
             Total = .fields("Valor_Cobro")
             Saldo = .fields("Valor_Cobro") * 100
             NumAut = Format$(MidStrg(.fields("Periodo"), 3, 2), "00") & Format$(.fields("Num_Mes"), "00") & Format$(ContadorTarjeta, "00")
             ...
             'Print #NumFileFacturas, NumAut;
             Print #NumFileFacturas, "00";
            'Transaccion/Secuencial
             Print #NumFileFacturas, NumAut;
             'Print #NumFileFacturas, Format(ContadorTarjeta, "000000");
''            MsgBox TRep.Fecha_Cad & vbCrLf & Format(TRep.Fecha_Cad, "YYYYMM") & vbCrLf & TRep.CI_RUC & " - " & MidStrg(TRep.CI_RUC, 5, 6)
...
'          Else
             Codigo4 = .fields("Producto") & " de " & .fields("Periodo") & "-" & MesesLetras(.fields("Num_Mes")) & " " & .fields("Grupo")
'          End If
''          If AdoAux.Recordset.RecordCount > 0 Then
...
Private Sub Toolbar1_ButtonClick(ByVal Button As MSComctlLib.Button)
  ...
  Select Case Button.key
    Case "Enviar"
         Enviar_Cobros
    Case "SubirAbonos"
         'MsgBox "Desktop Test: " & TextoBanco
         Select Case TextoBanco
           Case "INTERNACIONAL", "OTROSBANCOS", "PICHINCHA", "PACIFICO", "TARJETAS"
                Subir_Abonos_erp
           Case Else
                Subir_Abonos
         End Select
         ...
         RatonReloj
         DGFactura.Visible = False
         DGFactura.Caption = "CARGA DE PREFACTURA DEL DIA POR " & TextoBanco
         TotalIngreso = 0
         With AdoFactura.Recordset
          If .RecordCount > 0 Then
              Do While Not .EOF
