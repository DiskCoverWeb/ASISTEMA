...
Public Sub Insertar_Mail(ListaMails As String, InsertarMail As String)
   ...
   If (InStr(ListaMails, Trim(InsertarMail)) = 0) And (InStr(InsertarMail, "@") > 0) And (Len(Trim(InsertarMail)) > 3) Then
...
Public Sub UnidadSistema()
  ...
  RutaSysTemp = LeftStrg(WindowsDirectory, 2) & "\SYSTEMP\"
  ChDir RutaSistema
End Sub
...
Dim Result As Boolean
   ...
   'MsgBox "Ruta: " & RutaArchivo & vbCrLf & "[" & Dir$(RutaArchivo) & "]"
    If Len(RutaArchivo) > 1 Then
       If Dir$(RutaArchivo) = "" Then Result = False Else Result = True
    Else
       Result = False
    End If
   'MsgBox "Ruta: " & RutaArchivo & vbCrLf & "[" & Result & "]"
