...
Public Function post_URL_JSon(cedula As String, MontoTotal As Currency, DiasMora As Integer) As Boolean
...
Dim DataJson As String
    ...
    If Len(URLToken) > 1 And Len(Token) > 1 Then
       DataJson = "{""cedula"":""" & cedula _
                & """, ""monto_total"":""" & Format(MontoTotal, "##0.00") _
                & """, ""dias_mora"":""" & Format(DiasMora, "##0") & """ }"
    
       Set postRequest = CreateObject("WinHttp.WinHttpRequest.5.1")
        
       postRequest.open "POST", URLToken, False
       ...
       Set p = JSON.parse(postRequest.responseText)
       Result = p.Item("status")
    End If
    post_URL_JSon = Result
    RatonNormal
'''    MsgBox "Parsed object output: " & JSON.toString(p) & vbCrLf _
'''         & "status: " & p.Item("status") & vbCrLf _
'''         & "message: " & p.Item("message") & vbCrLf _
'''         & "Error: " & p.Item("error") & vbCrLf _
'''         & "Data: " & p.Item("data") & vbCrLf
End Function
