...
Private Sub Command1_Click()
  ...
  Mensajes = "Esta Seguro que desea convertir Tickes por " & FA.TC & ", No. " & Numero_Factura(FA)
            ...
            'MsgBox Ln_No & vbCrLf & Cant_Item_FA
             If (Ln_No >= Cant_Item_FA) Or (Mifecha <> .Fields("Fecha")) Or (FA.CodigoC <> .Fields("CodigoC")) Then
               ...
               'Limpiamos para otra factura/Nota de Venta
                ...
                If OpcResumen.Value Then FA.CodigoC = .Fields("CodigoC") Else FA.CodigoC = CodigoCli
                Mifecha = .Fields("Fecha")
                If Cant_Item_FA <= 0 Then Cant_Item_FA = 10
             End If
             Contador = Contador + 1
...
Private Sub Command4_Click()
  If OpcIndividual.Value Then
     ...
     If Grafico_PV Then
        Imprimir_Punto_Venta_Grafico AdoFactura, AdoIngCaja, FA
     Else
        Imprimir_Punto_Venta AdoFactura, AdoIngCaja, FA
     End If
  Else
...
Private Sub DCLinea_LostFocus()
  FA.Cod_CxC = DCLinea
  Lineas_De_CxC FA
  Label4.Caption = " No. " & FA.Serie & "-" & Format(Numero_Factura(FA), "0000000")
End Sub
...
Private Sub Form_Activate()
  ...
  SelectDBCombo DCTicket, AdoTicket, sSQL, "Ticket"
...
Public Sub Caja_Pendiente_PV()
  ...
  SelectDataGrid DGFactura, AdoFactura, sSQL
  RatonReloj
  Total = 0: Contador = 0
  With AdoFactura.Recordset
   If .RecordCount > 0 Then
