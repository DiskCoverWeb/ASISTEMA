...
Private Sub Command1_Click()
 ...
 'Verificacion de Cuentas Contables en Abonos en Cta
  ...
  sSQL = "SELECT TC,Serie,Factura,Autorizacion,Total_MN,(Con_IVA+Sin_IVA+IVA-Descuento) As SubTotales " _
       & "FROM Facturas " _
       & "WHERE Item = '" & NumEmpresa & "' " _
       & "AND Periodo = '" & Periodo_Contable & "' " _
       & "AND Fecha BETWEEN #" & FechaIni & "# and #" & FechaFin & "# " _
       & "AND Total_MN-(Con_IVA+Sin_IVA+IVA-Descuento) <> 0 " _
       & "AND NOT TC IN('C','P','OP') " _
       ...
       If BoxMensaje = vbYes Then
         ...
         'Grabacion del Comprobante de CxC
          If AdoAsiento1.Recordset.RecordCount > 0 Then
             ...
             Co.Item = NumEmpresa
             GrabarComprobante Co, AdoAsiento1
...
'''   If SQL_Server Then
'''      sSQL = "UPDATE Trans_Abonos " _
'''           & "SET Cta_CxP = F.Cta_CxP " _
'''           & "FROM Trans_Abonos As TA,Facturas As F "
'''   Else
'''      sSQL = "UPDATE Trans_Abonos As TA,Facturas As F " _
'''           & "SET TA.Cta_CxP = F.Cta_CxP "
'''   End If
...
'''   ConectarAdoExecute sSQL
 ...
 ' Ventas Anticipadas
  ...
  'Asientos de CxC Cheque
   ...
   sSQL = "SELECT TA.TP,TA.Fecha,C.Cliente,TA.Serie,TA.Autorizacion,TA.Factura,TA.Banco,TA.Cheque,TA.Abono,TA.Comprobante,TA.Cta,TA.Cta_CxP " _
  ...
  'Asientos de CxC Efectivo
   ...
   sSQL = "SELECT F.TC,F.Fecha,C.Cliente,F.Serie,F.Autorizacion,F.Factura,F.IVA As Total_IVA,F.Descuento,F.Servicio,F.Total_MN,F.Saldo_MN,F.Cta_CxP,A.Nombre_Completo As Ejecutivo " _
        & "FROM Facturas F,Clientes C,Accesos As A " _
        & "WHERE F.Fecha BETWEEN #" & FechaIni & "# and #" & FechaFin & "# " _
        & "AND F.TC NOT IN ('C','P','OP') " _
        & "AND F.T <> 'A' " _
  ...
  'Asiento de Salida de Inventario y Ventas del dia
...
'       & "F.Total_MN As TOTAL,Serie_R,Retencion,Ret_Fuente,Ret_IVA "
  ...
  sSQL = "SELECT F.TC,F.T,F.RUC_CI,F.Razon_Social,F.Fecha,F.Hora,A.Nombre_Completo As Usuario," _
       ...
       & "F.Sin_IVA As Base_0,F.Descuento,F.SubTotal,F.IVA As IVA_" & Codigo & ",F.Servicio,F.Total_MN As TOTAL," _
  ...
  LblDescuento.Caption = Format(Total_Desc, "#,##0.00")
