...
Begin VB.Form IngMarcas 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Ingreso/Modificacion de Marcas"
   ClientHeight    =   2880
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   5580
...
Private Sub DCInv_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If KeyCode = vbKeyF1 Then GenerarDataTexto IngMarcas, AdoInv
...
Public Sub LlenarInv()
   ...
   With AdoInv.Recordset
    If .RecordCount > 0 Then
        Codigo = SinEspaciosIzq(DCInv.Text)
       .MoveFirst
        TextoBusqueda = "CodMar Like '" & Codigo & "' "
       .Find (TextoBusqueda)
        If Not .EOF Then
           TextSubCta.Text = .Fields("Marca")
           TxtCodigo.Text = .Fields("CodMar")
        End If
    Else
        Nuevo = True
        TextSubCta.SetFocus
...
Public Sub GrabarInv()
  ...
  With AdoInv.Recordset
   ...
   Else
      ...
      Nuevo = True
   End If
   SetFields AdoInv, "CodMar", Codigo
   SetFields AdoInv, "Marca", UCase(TextSubCta)
...
Public Sub ListarMarcas(OpcVista As String)
   sSQL = "SELECT CodMar & '  ' & Marca As NomProd,* " _
        & "FROM Catalogo_Marcas " _
        & "WHERE Item = '" & NumEmpresa & "' " _
        & "AND Periodo = '" & Periodo_Contable & "' " _
        & "AND CodMar <> '.' " _
